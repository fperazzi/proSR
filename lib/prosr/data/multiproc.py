    ...
    def __next__(self):
        if self.num_workers == 0:  # same-process loading
            ...
            if self.samples_remaining == 0:
                raise StopIteration
            if self.random_vars and isinstance(self.sampler, RandomSampler):
    ...
    def _put_indices(self):
        ...
        if self.samples_remaining > 0:
            if self.samples_remaining < self.batch_size and self.drop_last:
                ...
                if self.random_vars:
                    self.index_queue.put((self.send_idx,
                                          random.choice(self.random_vars),
                                          self._next_indices()))
                else:
                    self.index_queue.put((self.send_idx, None,
                                          self._next_indices()))
                self.batches_outstanding += 1
                self.send_idx += 1
